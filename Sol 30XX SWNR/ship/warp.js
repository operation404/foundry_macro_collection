...
if (controlled_tokens.length > 0){
	controlled_tokens.forEach(make_warp);
} else {
	ui.notifications.warn("No Tokens were selected");
}

function make_warp(token, index) {
    ...
    const warp_duration = 15;
    const life_support_consumed = attr.crew.value * warp_duration;
	...
	game.Gametime.advanceClock(warp_duration * 86400); // 86400 is seconds in a day
    ...
    let warp_html = `<span style="float: left;">${token.data.name} makes a warp.</span></br>
                            ...
                            <span style="float: left;">Time passed: ${warp_duration} day.</span>`;
